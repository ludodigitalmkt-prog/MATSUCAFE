...
// 🔴 E-MAIL DO DONO AQUI (Libera exclusão e configurações) 🔴
const ADMIN_EMAIL = "gestao@matsu.com"; 

// Variável Global para segurar os dados da empresa e usar no recibo
let systemSettings = {
...
// ==========================================
// 1. LEITOR DE CÓDIGO DE BARRAS (INFRAVERMELHO)
// ==========================================
...
function handleBarcodeScan(code) {
    if (!document.getElementById('tab-menu').classList.contains('hidden')) {
        document.getElementById('prod-barcode').value = code; return;
    }
    const product = products.find(p => p.barcode === code);
    if (product) {
        addToCart(product);
        const bg = document.body.style.backgroundColor;
        document.body.style.backgroundColor = '#dcfce3'; setTimeout(() => document.body.style.backgroundColor = bg, 150);
    } else alert("Produto não encontrado: " + code);
}

// ==========================================
// 2. CONFIGURAÇÕES ERP E LOGIN
// ==========================================
async function loadSettings() {
    ...
    if (configDoc.exists()) {
        ...
        // Popula os inputs da tela de configurações
        ...
        // Atualiza a variável global pro Recibo ler depois
        systemSettings = { ...systemSettings, ...dados };
    }
}

// Botão GIGANTE de Salvar Configurações
document.getElementById('btn-save-settings').addEventListener('click', async () => {
    ...
    // Lida com o Upload da Logo separadamente (se o usuário escolheu uma nova imagem)
    ...
    if (file) {
        ...
        } catch (e) { alert("Erro ao subir a logo."); }
    }

    // Salva tudo no banco
    await setDoc(doc(db, "configuracoes", "loja"), dados, { merge: true });
    
    // Recarrega pra aplicar cor e logo
    await loadSettings(); 
...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        if (user.email === ADMIN_EMAIL) {
            ...
            initDashboard(); // Inicia o dash (que tem o botão de apagar venda)
        } else {
...
function loadCRM() {
    onSnapshot(collection(db, "clientes"), (snapshot) => {
        ...
        const select = document.getElementById('pdv-cliente'); select.innerHTML = '<option value="">Cliente Padrão (Avulso)</option>';
        snapshot.forEach((docSnap) => {
            const c = { id: docSnap.id, ...docSnap.data() }; clients.push(c);
...
document.getElementById('btn-add-crm').addEventListener('click', async () => {
    const nome = document.getElementById('crm-nome').value; const tel = document.getElementById('crm-telefone').value; const tipo = document.getElementById('crm-tipo').value;
    if(!nome) return alert("Preencha o nome!");
    await addDoc(collection(db, "clientes"), { nome, telefone: tel, cpf: document.getElementById('crm-cpf').value, tipo, dataCadastro: serverTimestamp() });
...
async function loadProducts() {
    onSnapshot(collection(db, "produtos"), (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
            ...
            div.addEventListener('click', () => addToCart(p)); grid.appendChild(div);
...
function updateCartUI() {
    ...
    if (cart.length === 0) cartItemsDiv.innerHTML = '<p class="text-center text-gray-400 mt-10 font-medium">Carrinho Vazio.</p>';
        ...
        const div = document.createElement('div'); div.className = 'flex justify-between items-center bg-white border border-gray-100 p-4 rounded-2xl shadow-sm';
...
document.getElementById('btn-checkout').addEventListener('click', async () => {
    ...
    try {
        await addDoc(collection(db, "vendas"), { nroPedido, itens: cart, total: cartTotal, cliente: cliente, pagamento: pagamento, data: serverTimestamp() });
    } catch(e) { return alert("Erro no banco."); }
    
    // GERA O RECIBO PUXANDO DADOS DAS CONFIGURAÇÕES
    let itemsHtml = '';
...
// ==========================================
// 5. FINANCEIRO (AGORA COM BOTÃO DE EXCLUIR VENDA)
// ==========================================
function initDashboard() {
    onSnapshot(collection(db, "vendas"), (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
            ...
            // Lógica para apagar a venda do sistema e recalcular
            div.querySelector('.btn-delete-sale').addEventListener('click', async () => {
                if(confirm(`Tem certeza que deseja EXCLUIR a Venda #${venda.nroPedido}?\nO valor será removido do Faturamento e do Caixa.`)) {
